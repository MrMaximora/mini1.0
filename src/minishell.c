...
void	loop_main_end(t_input line, t_execute *process, t_init *init_shell, t_lexer_tab	*tab_lexer)
{
	...
	else if (process->n_pipe >= 0 && line.all_tokens[0])
...
int	set_file(t_execute *process, t_pipe *pipes, t_lexer_tab *current, t_init *init_shell)
{
	...
	{
		if (access(current->next->arg, F_OK) == -1)
		{
				ft_putstr_fd(" No such file or directory\n", 2);
	...
	if (current->type == TOKEN_REDIRECT_OUTPUT)
	{
		...
		{
			if (access(current->next->arg, W_OK) != 0)
			{
	...
	if (current->type == TOKEN_ARGS)
	{
		...
		{	
			if (access(current->arg + 1, F_OK) == -1)
			{
				ft_putstr_fd(" No such file or directory\n", 2);
				current->infile = 2;
				init_shell->exit_code = 1;
			...
			//pipes[i].fdin_str = current->arg + 1;
			try_close(pipes[i].saved_fdin);
			pipes[i].saved_fdin = pipes[i].fdin;
			current->infile = 1;
		}
		...
		{
			if (access(current->arg + 1, F_OK) == 0)
			{
				if (access(current->arg + 1, W_OK) != 0)
				{
					ft_putstr_fd(" Permission denied\n", 2);
					init_shell->exit_code = 1;
					return (-1);
		...
		{
			//ft_printf("current->arg apppend = %s\n", current->arg + 2);
			if (access(current->arg + 2, F_OK) == 0)
			{
				if (access(current->arg + 2, W_OK) != 0)
				{
					ft_putstr_fd(" Permission denied\n", 2);
					init_shell->exit_code = 1;
					return (-1);
		...
		else if (current->infile == 3 && current->arg[0] == '.')
		{
			if (access(current->arg, R_OK) == -1)
			{
				ft_putstr_fd(" Permission denied\n", 2);
				init_shell->exit_code = 126;
				return (-1);
			}
			else if (access(current->arg, X_OK) == -1)
			{
				ft_putstr_fd(" Permission denied\n", 2);
				init_shell->exit_code = 126;
				return (-1);
...
void	pipeline_execute(t_input *line, t_execute *process, t_init *init_shell, t_lexer_tab *tab_lexer)
{
	...
	while (current)
	{
		...
		{
			if ((init_shell->exit_code == 1 && process->n_pipe == 0) || 
			(init_shell->exit_code == 1 && process->n_pipe == process->i) || 
			init_shell->exit_code != 1)
				return ;
		}
...
char	**get_command_args(t_input *line, t_lexer_tab **lexer_tab)
{
	...
	while (current && current->outfile == 0 && current->infile == 0
